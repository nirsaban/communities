...
// • Manager mode (PRD 06 §4 · screen 46): title is "Q&A · N open", filter icon
//   in the top-right, unanswered questions float to the top with an accent
  ...
  return (
    <Screen>
      <AppBar
        ...
        trailing={
          isManager ? (
            <ManagerFilterMenu value={mgrFilter} onChange={setMgrFilter} />
          ) : undefined
        }
      />
    ...
    <Card className="p-3.5">
      <div className="mb-2 flex items-center gap-2.5">
        ...
        <div className="flex-1">
          <div className="t-label-lg">Community member</div>
    ...
    <div className="card" style={cardStyle}>
      ...
      >
        ...
        <div className="grow flex-1 min-w-0">
          <div className="t-label-lg truncate">Community member</div>
        </div>
        {unanswered && (
          <span
...
function ManagerFilterMenu({
  value,
  onChange,
}: {
  value: ManagerFilter;
  onChange: (v: ManagerFilter) => void;
}) {
            ...
            {(['open', 'all', 'resolved'] as ManagerFilter[]).map((id) => (
              <button
                ...
                style={{
                  ...
                  padding: '8px 10px',
                  borderRadius: 8,
                  background: value === id ? 'rgb(var(--surface-2))' : 'transparent',
                  border: 0,
                  cursor: 'pointer',
                  fontSize: 14,
                }}
              >
                {id === 'open' ? 'Open' : id === 'resolved' ? 'Resolved' : 'All'}
              </button>
            ))}
          </div>
        </>
